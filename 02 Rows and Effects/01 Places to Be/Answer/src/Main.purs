...
--       (Highschool Maths is back. Import the Data.Int and Math modules,
--        specifically Data.Int.toNumber, and Math.sqrt, Math.abs and Math.pow)
--
--      Use the "pythag" helper at the bottom of the file if you get stuck.
distance :: Point -> Point -> Number
distance p1 p2 =
  let xDistance = Math.abs (Int.toNumber (p2.x - p1.x))
      yDistance = Math.abs (Int.toNumber (p2.y - p1.y))
      side n    = Math.pow n 2.0
   in Math.sqrt (side xDistance + side yDistance)

distanceWithHelper :: Point -> Point -> Number
distanceWithHelper p1 p2 =
  let xDistance = p2.x - p1.x
      yDistance = p2.y - p1.y
   in pythag xDistance yDistance



-- TODO: Harder: Write a function that takes an array of records containing *at least*
